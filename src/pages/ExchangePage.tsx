...
const ExchangePage = () => {
  ...
  const handleExchangeSuccess = (
    ...
    setTimeout(() => {
      ...
      toast.success("Ваш запрос на обмен успешно отправлен!", {
        description: `Номер вашего заказа: ${currentOrderId}. Вы обменяли ${formData.usdtAmount} USDT на ${formData.calculatedVND.toLocaleString(
          "vi-VN",
        )} VND.`,
        duration: 10000,
      });
  ...
  return (
    ...
    >
      ...
      <Card className="w-full max-w-lg mx-auto shadow-2xl rounded-2xl overflow-hidden relative z-10 bg-white/75 backdrop-blur-sm border-4 border-white/60 overflow-hidden rounded-none">
