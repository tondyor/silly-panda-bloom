...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Step 3: User data parsed. Telegram ID: ${user.id}, Username: ${user.username || 'N/A'}`);
    ...
    // Upsert профиля пользователя Telegram
    const { data: profile, error: upsertProfileError } = await supabase
      ...
      .upsert({
        ...
        // 'id' будет сгенерирован автоматически благодаря DEFAULT gen_random_uuid()
        // или будет использован существующий 'id' при обновлении
      }, { onConflict: 'telegram_id', ignoreDuplicates: false }) // ignoreDuplicates: false ensures update on conflict
      .select()
      .single();

    if (upsertProfileError) {
      console.error("Database Error: Failed to upsert Telegram profile.", upsertProfileError);
      throw new Error(`Ошибка базы данных при обновлении профиля: ${upsertProfileError.message}`);
    }
